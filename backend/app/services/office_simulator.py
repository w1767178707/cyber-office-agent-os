...
class OfficeAgentSimulator:
    ...
    def __init__(self, profiles: list[NPCProfile], states: StateManager, events: EventBus, memory: MemoryStore, tasks: TaskManager, llm: LLMClient, settings: Settings, compactor: MemoryCompactor | None=None):
        self.profiles = profiles
        self.profile_map = {p.npc_id: p for p in profiles}
        self.states = states
        self.events = events
        self.memory = memory
        self.tasks = tasks
    ...
    async def tick(self, use_llm_planner: bool | None=None) -> dict:
        ...
        if self.tick_count == 1:
            created_event_ids.append(self.events.publish('office_boot', 'CyberOffice 已进入全员并行 LLM 决策模式：每个空闲 Agent 会独立请求模型生成下一段行动。', []))
        ...
        return {'tick': self.tick_count, 'events': events, 'npc_states': self.states.list_states(), 'agent_traces': traces, 'tasks': self.tasks.list_tasks(limit=24)}
    ...
    async def _llm_episode_decision(self, profile: NPCProfile, state: dict, task: dict) -> EpisodeDecision | None:
        ...
        office_memories = self.memory.search_memories(profile.npc_id, '__office__', query_text, limit=6)
        player_memories = []
        ...
        context = {'tick': self.tick_count, 'agent': {'npc_id': profile.npc_id, 'name': profile.name, 'role': profile.role, 'department': profile.department, 'goals': profile.goals, 'skills': profile.skills, 'responsibilities': profile.responsibilities, 'personality': profile.personality}, 'state': {'location': state.get('location'), 'energy': state.get('energy'), 'stress': state.get('stress'), 'social_need': state.get('social_need'), 'focus': state.get('focus'), 'recent_zones': state.get('recent_zones') or [], 'coffee_streak': state.get('coffee_streak', 0), 'player_influence': influence}, 'task': task, 'relevant_office_memories': office_memories, 'relevant_player_memories': player_memories, 'recent_events': recent_events, 'teammates': self._teammate_snapshot(profile.npc_id), 'available_zones': zones, 'decision_contract': {'independent_agent_call': True, 'parallel_batch_tick': self.tick_count, 'you_control_only': profile.name, 'must_create_new_episode': True}}
        result = await self.llm.chat([{'role': 'system', 'content': '你是初创互联网公司办公室里的一个独立 Agent 决策器，此次调用只负责一个成员，不代表全局调度器。请基于该成员的岗位目标、当前状态、任务、队友动态、近期事件和办公室空间 affordance，自主生成下一段行动。不要从固定动作枚举中选择，也不要让所有人做同一件事；要体现真实互联网公司的分工协作。如果 state.player_influence.active=true，说明玩家刚刚与该成员对话并试图影响其行为；除非该指令不安全或与角色完全无关，否则下一段行动必须明显响应这个影响，并在 reasoning_factors 中写明。请结合 relevant_office_memories 与 relevant_player_memories，不要忘记摘要记忆中的长期偏好和承诺。只输出合法 json 对象，不要 Markdown，不要代码块。字段必须包含：intention, action, target_zone, selected_tool, mood, duration_ticks, thought, goal, reasoning_factors, confidence。json 示例：{"intention":"推进架构评审","action":"在白板区梳理服务边界并标注风险点","target_zone":"architecture_wall","selected_tool":"architecture_review","mood":"focused","duration_ticks":7,"thought":"当前任务需要技术负责人先澄清边界。","goal":"产出一个可执行的调整方案","reasoning_factors":["岗位职责匹配","任务处于阻塞状态"],"confidence":0.86}target_zone 必须来自 available_zones。除非 energy < 45 或角色明确需要非正式交流，不要把 target_zone 设为 coffee_bar；如果 recent_zones 最近连续去过同一地点，应主动换到角色相关工作区。duration_ticks 为 4-12 的整数。action 用中文，像真实互联网公司员工正在做的具体动作。回答尽量短，json 中每个字段不要长篇解释；reasoning_factors 用 2-5 条中文短语说明依据。'}, {'role': 'user', 'content': json.dumps(context, ensure_ascii=False)}], temperature=float(getattr(self.settings, 'office_llm_decision_temperature', 0.72) or 0.72), max_tokens=int(getattr(self.settings, 'office_llm_decision_max_tokens', 600) or 600), response_format={'type': 'json_object'}, call_purpose=f'parallel_episode_decision:{profile.npc_id}')
        if result.degraded:
    ...
    def _local_generative_decision(self, profile: NPCProfile, state: dict, task: dict) -> EpisodeDecision:
        ...
        return EpisodeDecision(intention=intention, tool=tool, action=action, mood=mood, target_zone=zone, thought=thought, goal=goal, duration_ticks=duration, confidence=confidence, reasoning_factors=factors[:6], source='local_generative', action_kind=self._infer_action_kind(action, tool, zone, state, task, motive=motive))
    ...
    async def _compact_memory_streams(self) -> None:
        if not self.compactor or not self._memory_keys_to_compact:
            return
        for npc_id, player_name in list(self._memory_keys_to_compact):
            ...
            if summary:
                ...
                self.events.publish('memory_compacted', f"{(profile.name if profile else npc_id)} 的 {player_name} 记忆流已压缩：{summary.get('source_count')} 条 → 摘要 #{summary.get('summary_id')}。", [npc_id])
        self._memory_keys_to_compact = set()
    ...
    def _trace_from_decision(self, profile: NPCProfile, decision: EpisodeDecision, target: Position, task: dict, state: dict) -> dict:
        return AgentDecision(npc_id=profile.npc_id, npc_name=profile.name, thought=decision.thought, goal=decision.goal, selected_tool=decision.tool, action=state.get('current_action') or decision.action, target_zone=decision.target_zone, target_position=target, task_id=task.get('id') if task else None, confidence=decision.confidence, phase=state.get('action_phase', 'walking'), intention=decision.intention, action_duration_ticks=decision.duration_ticks, action_remaining_ticks=state.get('action_remaining_ticks', decision.duration_ticks), action_progress=state.get('action_progress', 0.0), decision_source=decision.source, reasoning_factors=decision.reasoning_factors).model_dump()
