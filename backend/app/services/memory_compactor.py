...
class MemoryCompactor:
    ...
    async def compact_all_over_limit(self, *, threshold: int | None=None, limit: int=80) -> list[dict[str, Any]]:
        threshold = int(threshold or self.settings.memory_compact_threshold)
        streams = self.memory.active_streams(threshold=threshold, limit=limit)
        summaries: list[dict[str, Any]] = []
        for stream in streams:
            summary = await self.maybe_compact(str(stream['npc_id']), str(stream['player_name']))
            if summary:
