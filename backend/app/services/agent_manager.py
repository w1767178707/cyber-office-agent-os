...
class AgentManager:
    ...
    def __init__(self, npc_config_path: Path, llm: LLMClient, memory: MemoryStore, relationships: RelationshipManager, states: StateManager | None=None, events: EventBus | None=None, compactor: MemoryCompactor | None=None, influence_ttl_ticks: int=8, interrupt_current_episode: bool=True):
    ...
    async def dialogue(self, *, session_id: str, npc_id: str, player_name: str, player_message: str) -> dict:
        ...
        if self.states:
            ...
            if influence:
                self.memory.add_memory(npc_id=npc_id, player_name=player_name, content=f"【玩家行为影响】{player_name} 对 {profile.name} 说：{player_message}\n系统提取指令：{influence.get('directive')}；目标地点倾向：{influence.get('target_zone_hint') or '无'}；影响强度：{influence.get('influence_score')}；原因：{influence.get('reason')}", importance=max(3, importance), kind='player_influence')
        if self.compactor:
